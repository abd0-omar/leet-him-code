...
struct NumberContainers {
    // a good follow up for yesterday's problem
...
impl NumberContainers {
    ...
    fn find(&self, number: i32) -> i32 {
        // println!("{:?}", self.number_to_indices);
        if let Some(b_tree_set) = self.number_to_indices.get(&number) {
            *b_tree_set.first().unwrap_or(&-1)
        } else {
            -1
        }
    }

...
fn main() {
    ...
    let matrix2 = vec![
        ...
        vec![
            -88, -38, -22, -90, 54, 42, -29, 67, -85, -90, -29, 81, 52, 35, 13, 61, -18, -94, 61,
            -62,
        ],
        vec![
            -23, -29, -76, -30, -65, 23, 31, -98, -9, 11, 75, -1, -84, -90, 73, 58, 72, -48, 30,
            -81,
        ],
        vec![
        ...
        vec![
            -3, 78, 36, -92, -52, -44, -66, -53, -55, 76, -7, 76, -73, 13, -98, 86, -99, -22, 61,
            100,
        ],
        vec![
        ...
        vec![
            32, 94, -14, -13, -6, -55, -21, 29, -21, 16, 67, 100, 77, -26, -96, 22, -5, -53, -92,
            -36,
        ],
        vec![
    ...
    // println!("{}", min_falling_path_sum(matrix2));
    println!("Hello, world!");
}
...
pub fn min_falling_path_sum(matrix: Vec<Vec<i32>>) -> i32 {
    ...
    for j in 0..matrix[0].len() {
        let result = dfs(&matrix, 0, j as i32, &directions, &mut min_sum, &mut memory);
        min_sum = min_sum.min(result);
    }
...
fn dfs(
    ...
    min_sum: &mut i32,
    memory: &mut HashMap<(i32, i32), i32>,
) -> i32 {
    if cellx == matrix.len() as i32 {
        return 0;
    }

    if let Some(&ret) = memory.get(&(cellx, celly)) {
        return ret;
    }

    let mut local_min_sum = i32::MAX;

    for (di, dj) in directions {
        let i = cellx + di;
        let j = celly + dj;

        if (0..matrix[0].len()).contains(&(j as usize)) {
            let result = matrix[cellx as usize][celly as usize]
                + dfs(matrix, i, j, directions, min_sum, memory);
            local_min_sum = local_min_sum.min(result);
        }
    }

    memory.insert((cellx, celly), local_min_sum);
    local_min_sum
}

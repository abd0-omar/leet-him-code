...
//                      st -> 0
//  -1, -2, 4, -7, 3                    -2, 4, -7, 3
//  goal is to reach (last index) with (maximum score)

pub fn _max_result_graph(nums: &Vec<i32>, k: usize) -> i32 {
    let mut pq = std::collections::BinaryHeap::new();
    pq.push((nums[0], 0));
    let mut distance = vec![i32::MIN; nums.len()];
    distance[0] = nums[0];

    while let Some((cur_val, cur_idx)) = pq.pop() {
        if cur_idx == nums.len() - 1 {
            return cur_val;
        }

        for neighbor_idx in cur_idx + 1..=cur_idx + k {
            if neighbor_idx < nums.len() {
                // relaxation
                //       20
                //  a -------- d
                //  |          x
                ...
                //  |     5    |
                //  b ---------
                let neighbor_val = nums[neighbor_idx];
                let total_val = neighbor_val + cur_val;
                if distance[neighbor_idx] == i32::MIN || distance[neighbor_idx] > total_val {
                    pq.push((total_val, neighbor_idx));
                    distance[neighbor_idx] = total_val;
                }
            }
        }
    }
